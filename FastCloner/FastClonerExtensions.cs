...
internal static class FastClonerExtensions
{
    ...
    /// </summary>
    public static T DeepClone<T>(this T obj) => FastClonerGenerator.CloneObject(obj);
    ...
    public static TTo DeepCloneTo<TFrom, TTo>(this TFrom objFrom, TTo objTo) where TTo : class, TFrom => (TTo)FastClonerGenerator.CloneObjectTo(objFrom, objTo, true);
    ...
    /// <returns>existing filled object</returns>
    /// <remarks>Method is valid only for classes, classes should be descendants in reality, not in declaration</remarks>
    public static TTo ShallowCloneTo<TFrom, TTo>(this TFrom objFrom, TTo objTo) where TTo : class, TFrom => (TTo)FastClonerGenerator.CloneObjectTo(objFrom, objTo, false);
